...
const Events={m:{},on(e,f){(this.m[e]=this.m[e]||[]).push(f);},emit(e,...a){(this.m[e]||[]).forEach(f=>f(...a));}};
...
const Store={
  ...
  buyItem(it){ if(this.data.coins<it.cost)return false; this.addCoins(-it.cost); this.data.items.push(it.id); this.data.equip[it.slot]=it.id; this.save(); return true; },
  toggleEquip(it){ this.data.equip[it.slot]=(this.data.equip[it.slot]===it.id)?null:it.id; this.save(); }
};
...
const Sfx={
  ...
  click(){this.tone(620,0.06,'square',0.09);}, jump(){this.tone(260,0.18,'square',0.11,240);},
  ...
  hit(){this.tone(150,0.22,'sawtooth',0.17,-90);}, trap(){this.tone(220,0.3,'sawtooth',0.17,-160);},
  ...
  tap(){this.tone(740,0.05,'triangle',0.09);}, splash(){this.tone(400,0.2,'sine',0.1,-200);},
  reel(){this.tone(800,0.05,'square',0.06,300);}, tick(){this.tone(1200,0.04,'square',0.08);},
