...
const Engine = {
  ...
  fxHalo:null, toolObj:null, _tool:null,
  ...
  /* Markette alınan kozmetikleri avatara takar */
  applyCosmetics(g){
    this.fxHalo=null;
  ...
  init(canvas){
    ...
    this.camOff=new THREE.Vector3(0,5.4,9);
    ...
    const resize=()=>{
      ...
      const asp=w/h;
      self.defCamY = asp<0.75 ? 6.2 : 5.4;
      self.defCamZ = asp<0.75 ? 11 : 9;
      self.camOff.set(0,self.defCamY,self.defCamZ);
    };
  ...
  spawnTrail(){
    ...
    let color='#ffd32a';
    if(this.trailType==='trail_rainbow'){
      color='#'+new THREE.Color().setHSL((this.time*0.4)%1,1,0.6).getHexString();
    }
  ...
  stepPlayer(dt){
    ...
    if(!this.noMove){
      ...
      P.vel.x=U.approach(P.vel.x, mx*P.speed, acc*dt);
      P.vel.z=U.approach(P.vel.z, -mf*P.speed, acc*dt);
    }
  ...
  updateCamera(dt){
    const P=this.player.pos;
    const target=new THREE.Vector3(P.x+this.camOff.x, P.y+this.camOff.y, P.z+this.camOff.z);
    ...
    this.camera.lookAt(P.x,P.y+1.15,P.z);
    return sh;
  },
  ...
  startGame(meta){
    ...
    this.camOff.set(0,this.defCamY,this.defCamZ);
    W.clear();
