...
const keys={};
const Input={
  jx:0,jf:0,btnJump:false,btnAct:false,jumpBuf:0,actionBuf:0,_pj:false,_pa:false,
  update(dt){
    this.jumpBuf=Math.max(0,this.jumpBuf-dt);
    this.actionBuf=Math.max(0,this.actionBuf-dt);
    const jh=this.btnJump||!!keys['Space']; if(jh&&!this._pj)this.jumpBuf=0.18; this._pj=jh;
    const ah=this.btnAct||!!keys['KeyE']||!!keys['Enter']; if(ah&&!this._pa)this.actionBuf=0.22; this._pa=ah;
  },
  axis(){
    ...
    const l=Math.hypot(x,f); if(l>1){x/=l;f/=l;} return {x:x,f:f};
  }
};

function bindInput(){
  const block=['Space','ArrowUp','ArrowDown','ArrowLeft','ArrowRight'];
  window.addEventListener('keydown',e=>{ if(block.indexOf(e.code)>=0)e.preventDefault(); keys[e.code]=true; });
  ...
  zone.addEventListener('pointerdown',e=>{e.preventDefault();joyId=e.pointerId;try{zone.setPointerCapture(e.pointerId);}catch(err){}zone.classList.add('on');joyMove(e);});
  zone.addEventListener('pointermove',e=>{if(e.pointerId===joyId)joyMove(e);});
  const joyEnd=e=>{if(e.pointerId!==joyId)return;joyId=null;Input.jx=0;Input.jf=0;thumb.style.transform='translate(0,0)';zone.classList.remove('on');};
...
function bindCamera(){
  ...
  zone.addEventListener('wheel',e=>{e.preventDefault();Engine.camDist=U.clamp(Engine.camDist+e.deltaY*0.01,3,22);},{passive:false});
  $('btn-cam').addEventListener('click',()=>{
    ...
    if(Engine.camMode!==2){
      const P=Engine.player;const hs=Math.hypot(P.vel.x,P.vel.z);
...
const FX={
  ...
  burst(x,y,z,color,n,spd,life){
    ...
    for(let i=0;i<n;i++){
      ...
      this.parts.push({m:m,vx:(Math.random()-0.5)*spd,vy:Math.random()*spd*0.7+2,vz:(Math.random()-0.5)*spd,life:life,tl:0});
    }
  },
  confetti(x,y,z){const cols=['#ff4d5e','#ffd32a','#3ecf5a','#2f7df6','#8a5cf6'];for(let i=0;i<5;i++)this.burst(x,y,z,cols[i],10,8,1.1);},
  floatText(x,y,z,text,color){
    ...
    sp.scale.set(2.4,0.9,1);sp.position.set(x,y,z);Engine.scene.add(sp);
    this.floats.push({m:sp,tl:0,life:1});
  },
  ring(x,y,z,color){
    const m=new THREE.Mesh(new THREE.RingGeometry(0.5,0.68,26),new THREE.MeshBasicMaterial({color:color,transparent:true,opacity:0.9,side:THREE.DoubleSide,depthWrite:false}));
    m.rotation.x=-Math.PI/2;m.position.set(x,y,z);Engine.scene.add(m);
    this.customs.push({m:m,tl:0,life:0.5,fn:(mm,e,l)=>{const k=1+e*16;mm.scale.set(k,k,k);mm.material.opacity=0.9*(1-e/l);}});
  },
  spawn(mesh,life,fn){this.customs.push({m:mesh,tl:0,life:life,fn:fn});},
  update(dt){
    for(let i=this.parts.length-1;i>=0;i--){const p=this.parts[i];p.tl+=dt;
    ...
    for(let i=this.floats.length-1;i>=0;i--){const f=this.floats[i];f.tl+=dt;
    ...
    for(let i=this.customs.length-1;i>=0;i--){const c=this.customs[i];c.tl+=dt;
      if(c.tl>=c.life){Engine.scene.remove(c.m);this.customs.splice(i,1);continue;}
      if(c.fn)c.fn(c.m,c.tl,c.life);}
  },
  clear(){const kill=a=>{a.forEach(o=>Engine.scene.remove(o.m));a.length=0;};kill(this.parts);kill(this.floats);kill(this.customs);}
};

const HUD={
  toastT:null,bigT:null,
  show(v){$('hud').classList.toggle('hidden',!v);},
  lobby(){$('hud-title').textContent='🏝️ LOBİ';document.querySelectorAll('.lobby-only').forEach(b=>b.style.display='');$('hud-hearts').classList.add('hidden');$('hud-stat').classList.add('hidden');HUD.boss(null);},
  game(m){$('hud-title').textContent=m.emoji+' '+m.name;document.querySelectorAll('.lobby-only').forEach(b=>b.style.display='none');$('hud-hearts').classList.add('hidden');$('hud-stat').classList.add('hidden');HUD.boss(null);},
  hearts(n,max){const el=$('hud-hearts');el.classList.remove('hidden');let s='';for(let i=0;i<max;i++)s+=i<n?'❤️':'🖤';el.textContent=s;},
  stat(t){const el=$('hud-stat');if(t){el.textContent=t;el.classList.remove('hidden');}else el.classList.add('hidden');},
  coins(){const c='🪙 '+Store.data.coins;['hud-coins','m-coins','c-coins','g-coins','s-coins'].forEach(id=>{const el=$(id);if(el)el.textContent=c;});},
  toast(t,d){d=d||1.5;const el=$('toast');el.textContent=t;el.classList.add('show');clearTimeout(this.toastT);this.toastT=setTimeout(()=>el.classList.remove('show'),d*1000);},
  big(t,color,d){d=d||1.2;const el=$('bigprompt');el.textContent=t;el.style.background=color||'#2f7df6';el.classList.remove('hidden');el.style.animation='none';void el.offsetWidth;el.style.animation='';clearTimeout(this.bigT);this.bigT=setTimeout(()=>el.classList.add('hidden'),d*1000);},
  interact(t){const el=$('interact');if(t){el.textContent='✋ '+t;el.classList.remove('hidden');}else el.classList.add('hidden');},
  setControls(v){$('controls').classList.toggle('hidden',!v);},
  boss(r){const el=$('bossbar');if(r==null){el.classList.add('hidden');return;}el.classList.remove('hidden');$('bossfill').style.width=Math.max(0,r*100)+'%';}
};
...
const W={
  ...
  box(x,y,z,w,h,d,color,o){
    o=o||{};
    let mat=(o.trigger||o.noStud)?Engine.MAT(color):Engine.MATSTUD(color,w,d);
    const m=new THREE.Mesh(Engine.GEO.box,mat);
    m.scale.set(w,h,d);m.position.set(x,y,z);
    if(o.trigger)m.visible=false;
    Engine.scene.add(m);Engine.items.push(m);
    const c={x:x,y:y,z:z,hx:w/2,hy:h/2,hz:d/2,mesh:m,trigger:!!o.trigger,disabled:false,tag:o.tag||null,cb:o.cb||null,onLand:null};
    Engine.colliders.push(c);return c;
  },
  mover(x,y,z,w,h,d,color,o){
    const c=this.box(x,y,z,w,h,d,color,o);
    c.base={x:x,y:y,z:z};c.axis=o.axis||'x';c.amp=o.amp||2;c.speed=o.speed||1;c.phase=o.phase||0;
    Engine.movers.push(c);return c;
  },
  mesh(m){Engine.scene.add(m);Engine.items.push(m);return m;},
  removeC(c){if(!c)return;c.disabled=true;if(c.mesh)Engine.scene.remove(c.mesh);
  ...
  clear(){
    ...
    FX.clear();Engine.clearTrail();this.killY=-30;Engine.setTool(null);
  }
};
...
const Engine={
  ...
  fxHalo:null,fxAura:null,pet:null,toolObj:null,_tool:null,
  ...
  studTex(n){
    ...
    for(let ix=0;ix<n;ix++)for(let iy=0;iy<n;iy++){
      ...
      g.beginPath();g.arc(cx,cy,r,0,Math.PI*2);g.fillStyle='#f4f4f4';g.fill();
      g.lineWidth=Math.max(2,cell*0.09);g.strokeStyle='rgba(0,0,0,0.20)';g.stroke();
      g.beginPath();g.arc(cx-r*0.25,cy-r*0.25,r*0.45,0,Math.PI*2);g.fillStyle='rgba(255,255,255,0.65)';g.fill();
    }
    const t=new THREE.CanvasTexture(cv);this._studTex[n]=t;return t;
  },
  MATSTUD(color,w,d){
    const n=U.clamp(Math.round(Math.max(w,d)/1.4),2,14);const key=color+'_'+n;
  ...
  buildSkyDome(top,horizon){
    ...
    const m=new THREE.Mesh(new THREE.SphereGeometry(150,16,12),new THREE.MeshBasicMaterial({map:tex,side:THREE.BackSide,fog:false,depthWrite:false}));
    m.renderOrder=-1;W.mesh(m);
  },
  buildSun(){
    if(!this._sunTex){const cv=document.createElement('canvas');cv.width=cv.height=128;const g=cv.getContext('2d');
      const gr=g.createRadialGradient(64,64,6,64,64,64);
      gr.addColorStop(0,'rgba(255,255,230,1)');gr.addColorStop(0.35,'rgba(255,240,170,0.9)');gr.addColorStop(1,'rgba(255,240,170,0)');
      g.fillStyle=gr;g.fillRect(0,0,128,128);this._sunTex=new THREE.CanvasTexture(cv);}
    const sp=new THREE.Sprite(new THREE.SpriteMaterial({map:this._sunTex,fog:false,depthWrite:false,transparent:true}));
    sp.scale.set(46,46,1);sp.position.set(70,95,-90);W.mesh(sp);
  },
  _faceCache:{},
  faceTex(skin){
    if(this._faceCache[skin])return this._faceCache[skin];
    const cv=document.createElement('canvas');cv.width=cv.height=64;const g=cv.getContext('2d');
    g.fillStyle=skin;g.fillRect(0,0,64,64);g.fillStyle='#10131f';
    g.fillRect(15,20,8,13);g.fillRect(41,20,8,13);
    g.lineWidth=4;g.strokeStyle='#10131f';g.beginPath();g.arc(32,42,11,0.25*Math.PI,0.75*Math.PI);g.stroke();
  ...
  textTex(t,fill){
    ...
    const w=Math.ceil(g.measureText(t).width)+28;cv.width=w;cv.height=64;
    const g2=cv.getContext('2d');g2.font='bold 44px Nunito, Arial Black';g2.textAlign='center';g2.textBaseline='middle';
  ...
  emojiTex(ch){
    const cv=document.createElement('canvas');cv.width=cv.height=128;const g=cv.getContext('2d');
    g.font='100px serif';g.textAlign='center';g.textBaseline='middle';g.fillText(ch,64,70);
    return new THREE.CanvasTexture(cv);
  },
  makeAvatar(g,c){
    const skin=this.MAT(c.skin),shirt=this.MAT(c.shirt),pants=this.MAT(c.pants);
    const mk=(sx,sy,sz,m,x,y,z)=>{const q=new THREE.Mesh(this.GEO.box,m);q.scale.set(sx,sy,sz);q.position.set(x,y,z);g.add(q);return q;};
  ...
  applyCosmetics(g){
    this.fxHalo=null;this.fxAura=null;
    const eq=Store.data.equip||{};
    if(eq.hat==='crown'){
      const gold=this.MAT('#ffd700');
      const base=new THREE.Mesh(new THREE.CylinderGeometry(0.3,0.34,0.16,10),gold);base.position.y=1.99;g.add(base);
      for(let i=0;i<5;i++){const sp=new THREE.Mesh(new THREE.ConeGeometry(0.07,0.2,6),gold);
        const a=i/5*Math.PI*2;sp.position.set(Math.cos(a)*0.24,2.15,Math.sin(a)*0.24);g.add(sp);}
    } else if(eq.hat==='tophat'){
      const blk=this.MAT('#15181d');
      const brim=new THREE.Mesh(new THREE.CylinderGeometry(0.5,0.5,0.06,14),blk);brim.position.y=1.95;g.add(brim);
      const top=new THREE.Mesh(new THREE.CylinderGeometry(0.3,0.3,0.5,14),blk);top.position.y=2.22;g.add(top);
      const band=new THREE.Mesh(new THREE.CylinderGeometry(0.31,0.31,0.1,14),this.MAT('#c0392b'));band.position.y=2.02;g.add(band);
    } else if(eq.hat==='halo'){
      const halo=new THREE.Mesh(new THREE.TorusGeometry(0.34,0.06,8,20),this.MATB('#ffd32a'));
      halo.rotation.x=Math.PI/2;halo.position.y=2.5;g.add(halo);this.fxHalo=halo;
    } else if(eq.hat==='viking'){
      const sil=this.MAT('#cfd8dc');
      const helm=new THREE.Mesh(new THREE.CylinderGeometry(0.32,0.36,0.3,10),sil);helm.position.y=2.0;g.add(helm);
      [[-1],[1]].forEach(s=>{const horn=new THREE.Mesh(new THREE.ConeGeometry(0.09,0.4,7),this.MAT('#efebe0'));
        horn.position.set(s[0]*0.36,2.15,0);horn.rotation.z=s[0]*-0.7;g.add(horn);});
    } else if(eq.hat==='wizard'){
      const pur=this.MAT('#7b2fbe');
      const brim=new THREE.Mesh(new THREE.CylinderGeometry(0.55,0.55,0.05,14),pur);brim.position.y=1.94;g.add(brim);
      const cone=new THREE.Mesh(new THREE.ConeGeometry(0.32,0.7,12),pur);cone.position.y=2.3;g.add(cone);
      const star=new THREE.Mesh(this.GEO.box,this.MATB('#ffd32a'));star.scale.setScalar(0.12);star.position.set(0,2.2,0.3);g.add(star);
    } else if(eq.hat==='cap'){
      const red=this.MAT('#e53935');
      const dome=new THREE.Mesh(new THREE.SphereGeometry(0.32,10,8,0,Math.PI*2,0,Math.PI/2),red);dome.position.y=1.93;g.add(dome);
      const brim=new THREE.Mesh(this.GEO.box,red);brim.scale.set(0.5,0.06,0.35);brim.position.set(0,1.94,0.4);g.add(brim);
    } else if(eq.hat==='demon'){
      [[-1],[1]].forEach(s=>{const horn=new THREE.Mesh(new THREE.ConeGeometry(0.09,0.45,7),this.MAT('#d32f2f'));
        horn.position.set(s[0]*0.24,2.12,0);horn.rotation.z=s[0]*-0.35;g.add(horn);});
    }
    if(eq.face==='glasses'){const gl=new THREE.Mesh(this.GEO.box,this.MAT('#15181d'));gl.scale.set(0.52,0.16,0.06);gl.position.set(0,1.72,0.27);g.add(gl);}
    else if(eq.face==='visor'){const v=new THREE.Mesh(this.GEO.box,this.MATB('#00e5ff'));v.scale.set(0.55,0.14,0.06);v.position.set(0,1.72,0.28);g.add(v);}
    else if(eq.face==='mask_ninja'){const m=new THREE.Mesh(this.GEO.box,this.MAT('#23272e'));m.scale.set(0.57,0.2,0.06);m.position.set(0,1.62,0.28);g.add(m);}
    if(eq.back==='wings'){
      const wm=this.MAT('#f8bbd0');
      [[-1,1],[1,-1]].forEach(s=>{const w=new THREE.Mesh(this.GEO.box,wm);w.scale.set(0.5,0.85,0.08);
        w.position.set(s[0]*0.58,1.3,-0.3);w.rotation.y=s[0]*0.55;w.rotation.z=s[0]*-0.18;g.add(w);});
    } else if(eq.back==='dragon_wings'){
      const wm=this.MAT('#b71c1c');
      [[-1,1],[1,-1]].forEach(s=>{const w=new THREE.Mesh(this.GEO.box,wm);w.scale.set(0.7,1.1,0.08);
        w.position.set(s[0]*0.68,1.35,-0.32);w.rotation.y=s[0]*0.5;w.rotation.z=s[0]*-0.25;g.add(w);});
    } else if(eq.back==='jetpack'){
      const gm=this.MAT('#90a4ae');
      [[-1],[1]].forEach(s=>{const t=new THREE.Mesh(new THREE.CylinderGeometry(0.16,0.16,0.7,8),gm);
      ...
      const c=new THREE.Mesh(this.GEO.box,this.MAT('#d32f2f'));c.scale.set(0.8,1.1,0.06);
  ...
  buildPet(id){
    ...
    const add=(sx,sy,sz,m,x,y,z)=>{const q=new THREE.Mesh(this.GEO.box,m);q.scale.set(sx,sy,sz);q.position.set(x,y,z);g.add(q);return q;};
    if(id==='pet_dog'){
      ...
      add(0.3,0.3,0.08,this.MAT('#ffca28'),-0.09,0.85,0.9);add(0.3,0.3,0.08,this.MAT('#ffca28'),0.09,0.85,0.9);
      add(0.5,0.6,0.08,this.MAT('#2e7d32'),-0.5,0.7,0);add(0.5,0.6,0.08,this.MAT('#2e7d32'),0.5,0.7,0);
  ...
  init(canvas){
    ...
    const dl=new THREE.DirectionalLight(0xfff2d6,0.75);dl.position.set(12,24,10);this.scene.add(dl);
    ...
    this.player={pos:new THREE.Vector3(0,HY,7),vel:new THREE.Vector3(),prev:new THREE.Vector3(),
      onGround:false,coyote:0,groundC:null,iframe:0,speed:7,jumpV:12.4};
    this.playerGroup=new THREE.Group();this.playerGroup.visible=false;this.scene.add(this.playerGroup);
    this.blob=new THREE.Mesh(new THREE.CircleGeometry(0.55,20),new THREE.MeshBasicMaterial({color:0x000000,transparent:true,opacity:0.28,depthWrite:false}));
    this.blob.rotation.x=-Math.PI/2;this.blob.visible=false;this.scene.add(this.blob);
    const self=this;
    const resize=()=>{const w=window.innerWidth,h=window.innerHeight;
  ...
  frame(now){
    ...
    FX.update(dt);this.updateTrail(dt);this.updateAvatar(dt);
    if(this.idle){this.renderer.render(this.scene,this.camera);}
    else{const sh=this.updateCamera(dt);this.renderer.render(this.scene,this.camera);if(sh)this.camera.position.sub(sh);}
  },

  spawnTrail(){
    const P=this.player.pos;let color='#ffd32a';
    ...
    this.scene.add(m);this.trailPs.push({m:m,tl:0,life:0.5});
  },
  updateTrail(dt){
    for(let i=this.trailPs.length-1;i>=0;i--){const p=this.trailPs[i];p.tl+=dt;
      if(p.tl>=p.life){this.scene.remove(p.m);p.m.material.dispose();this.trailPs.splice(i,1);continue;}
      p.m.position.y+=dt*0.8;p.m.material.opacity=1-p.tl/p.life;p.m.scale.multiplyScalar(1-dt*1.5);}
  },
  clearTrail(){this.trailPs.forEach(p=>{this.scene.remove(p.m);p.m.material.dispose();});this.trailPs.length=0;this.trailType=null;},

  setTool(type){
    if(this.toolObj){if(this.toolObj.parent)this.toolObj.parent.remove(this.toolObj);this.toolObj=null;}
    this._tool=type;
    ...
    if(type==='sword'){
      ...
      const blade=new THREE.Mesh(this.GEO.box,bladeMat);blade.scale.set(0.12,1.15,0.3);blade.position.set(0,0.42,0);s.add(blade);
      const guard=new THREE.Mesh(this.GEO.box,this.MAT('#ffd32a'));guard.scale.set(0.16,0.12,0.44);s.add(guard);
    } else if(type==='pick'){
      const handle=new THREE.Mesh(this.GEO.box,this.MAT('#6d4c41'));handle.scale.set(0.12,1.1,0.12);s.add(handle);
      const head=new THREE.Mesh(this.GEO.box,this.MAT('#b0bec5'));head.scale.set(0.75,0.2,0.2);head.position.y=0.5;s.add(head);
    } else if(type==='rod'){
      const handle=new THREE.Mesh(this.GEO.box,this.MAT('#6d4c41'));handle.scale.set(0.1,1.3,0.1);s.add(handle);
      const tip=new THREE.Mesh(this.GEO.box,this.MAT('#e53935'));tip.scale.set(0.08,0.2,0.08);tip.position.y=0.7;s.add(tip);
    }
  ...
  stepPlayer(dt){
    ...
    if(!this.inputLock){
      ...
      if(Input.jumpBuf>0&&(P.onGround||P.coyote>0)){P.vel.y=P.jumpV;Input.jumpBuf=0;P.coyote=0;P.groundC=null;Sfx.jump();}
    }
    ...
    if(landed){P.onGround=true;P.groundC=landed;P.coyote=0.13;
      if(P.vel.y<0){if(P.vel.y<-13){FX.burst(P.pos.x,P.pos.y-HY,P.pos.z,'#cfd6dd',6,3,0.4);Sfx.land();}P.vel.y=0;}}
  ...
  updateMovers(dt){
    for(let i=0;i<this.movers.length;i++){const c=this.movers[i];
      const v=c.base[c.axis]+Math.sin(this.time*c.speed+c.phase)*c.amp;
      const delta=v-c[c.axis];c[c.axis]=v;c.mesh.position[c.axis]=v;
      if(this.player.groundC===c&&this.player.onGround)this.player.pos[c.axis]+=delta;}
  },
  checkTriggers(){
    if(!this.playerOn)return;const P=this.player;
    for(let i=0;i<this.colliders.length;i++){const c=this.colliders[i];
      if(!c.trigger||c.disabled||!c.cb)continue;
      if(Math.abs(P.pos.x-c.x)<c.hx+HX&&Math.abs(P.pos.y-c.y)<c.hy+HY&&Math.abs(P.pos.z-c.z)<c.hz+HZ)c.cb(c);}
  },
  handleFall(){ if(this.onFallCb)this.onFallCb(); else this.spawnPlayer(this.spawnPt.x,this.spawnPt.y,this.spawnPt.z,false); },
  updateInteract(){
    if(this.inputLock||!this.playerOn||this.idle){HUD.interact(null);return;}
    const P=this.player.pos;let best=null,bd=1e9;
    for(let i=0;i<this.interact.length;i++){const it=this.interact[i];if(it.dead)continue;
      const d=Math.hypot(P.x-it.x,P.z-it.z);if(d<it.r&&d<bd){bd=d;best=it;}}
    if(best){HUD.interact(typeof best.label==='function'?best.label():best.label);
  ...
  applyChar(ci){
    ...
    if(pid){const pg=this.buildPet(pid);if(pg){this.scene.add(pg);this.pet={grp:pg};}}
  },

  updateAvatar(dt){
    const g=this.playerGroup,P=this.player;
    g.visible=this.playerOn;this.blob.visible=false;
    ...
    if(this.punch>0){this.punch-=dt;this.limbs.armR.rotation.x=-2.4*Math.sin(U.clamp(1-this.punch/0.25,0,1)*Math.PI);}
    if(hs>0.6){
      const target=Math.atan2(P.vel.x,P.vel.z);
      g.rotation.y=lerpAngle(g.rotation.y,target,Math.min(1,dt*12));
    }
    if(P.iframe>0)g.visible=(Math.floor(this.time*14)%2)===0;
    if(this.fxHalo)this.fxHalo.rotation.z+=dt*2;
    if(this.fxAura){this.fxAura.rotation.z+=dt*2;
      const eq=Store.data.equip||{};
      if(eq.aura==='aura_rainbow')this.fxAura.material.color.setHSL((this.time*0.4)%1,1,0.6);}
    if(this.trailType&&hs>1.5){this.trailT-=dt;if(this.trailT<=0){this.trailT=0.07;this.spawnTrail();}}
    if(this.pet){
    ...
    for(let i=0;i<this.colliders.length;i++){const c=this.colliders[i];
      if(c.disabled||c.trigger)continue;
      const top=c.y+c.hy;
      if(top<=P.pos.y-HY+0.06&&top>best&&Math.abs(P.pos.x-c.x)<c.hx+0.6&&Math.abs(P.pos.z-c.z)<c.hz+0.6)best=top;}
    if(best>-1e8){this.blob.visible=true;this.blob.position.set(P.pos.x,best+0.03,P.pos.z);
      const hgt=P.pos.y-HY-best;this.blob.scale.setScalar(U.clamp(1-hgt*0.05,0.35,1));}
  },
  ...
  updateCamera(dt){
    ...
    let sh=null;
    if(this.shakeT>0){this.shakeT-=dt;const m=this.shakeM*Math.max(0,this.shakeT)*3;
      sh=new THREE.Vector3(U.rand(-m,m),U.rand(-m,m),0);this.camera.position.add(sh);}
  ...
  fadeDo(cb){const f=$('fade');f.classList.add('on');setTimeout(()=>{cb();setTimeout(()=>f.classList.remove('on'),80);},230);},
  ...
  buildLobby(){
    ...
    coin.rotation.z=Math.PI/2;cg.add(coin);W.mesh(cg);this.lobbyCoin=cg;
    const booth=new THREE.Group();booth.position.set(0,0,-5.6);
    const counter=new THREE.Mesh(this.GEO.box,this.MAT('#ffb100'));counter.scale.set(2.6,1.1,1.2);counter.position.y=0.55;booth.add(counter);
    ...
    W.mesh(booth);this.marketGem=gem;
    this.interact.push({x:0,z:-4.4,r:2.6,label:'💎 MARKET',cb:()=>Events.emit('reqShop')});
    this.emoSprites=[];
    const n=GAMES.length;const RAD=n>12?13.5:10.6;
    for(let i=0;i<n;i++){
      const gmeta=GAMES[i];const a=i/n*Math.PI*2;
      const px=Math.sin(a)*RAD,pz=-Math.cos(a)*RAD;
      const grp=new THREE.Group();grp.position.set(px,0,pz);grp.rotation.y=Math.atan2(-px,-pz);
      const mk=(sx,sy,sz,x,y,z,mm)=>{const q=new THREE.Mesh(this.GEO.box,mm);q.scale.set(sx,sy,sz);q.position.set(x,y,z);grp.add(q);};
      mk(0.55,3.1,0.55,-1.15,1.55,0,this.MAT(gmeta.color));
      mk(0.55,3.1,0.55,1.15,1.55,0,this.MAT(gmeta.color));
      mk(2.9,0.7,0.6,0,3.4,0,this.MAT(gmeta.color));
      const pad=new THREE.Mesh(this.GEO.box,this.MATB(gmeta.color));pad.scale.set(2.2,0.12,1.6);pad.position.set(0,0.07,2.1);grp.add(pad);
      ...
      this.interact.push({x:px+fx*2.1,z:pz+fz*2.1,r:2.7,label:'▶ '+gmeta.name,cb:(id=>()=>Events.emit('reqGame',id))(gmeta.id)});
    }
    this.clouds=[];
    for(let i=0;i<6;i++){const cl=new THREE.Mesh(this.GEO.box,this.MAT('#ffffff'));
      cl.scale.set(U.rand(3,6),1,U.rand(1.6,2.6));cl.position.set(U.rand(-30,30),U.rand(12,17),U.rand(-30,30));
      W.mesh(cl);this.clouds.push({m:cl,v:U.rand(0.4,1)});}
    for(let i=0;i<3;i++){const b=Bots.make(null,U.rand(-8,8),U.rand(-4,6));
      b.mode='wander';b.wander={cx:0,cz:0,r:11,tx:U.rand(-6,6),tz:U.rand(-6,6),wait:U.rand(0,2)};
      b.speed=U.rand(3,4.2);b.respawnPos.set(b.pos.x,0,b.pos.z);}
    this.spawnPt={x:0,y:HY+0.02,z:7.5};
  },
  lobbyUpdate(dt){
    if(this.lobbyCoin){this.lobbyCoin.rotation.y+=dt*2.2;this.lobbyCoin.position.y=2.7+Math.sin(this.time*2)*0.15;}
    if(this.marketGem)this.marketGem.position.y=2.1+Math.sin(this.time*2.4)*0.18;
    for(let i=0;i<this.emoSprites.length;i++){const e=this.emoSprites[i];e.sp.position.y=4.6+Math.sin(this.time*2+e.ph)*0.25;}
    for(let i=0;i<this.clouds.length;i++){const c=this.clouds[i];c.m.position.x+=c.v*dt;if(c.m.position.x>40)c.m.position.x=-40;}
    if(this.idle){const a=this.time*0.1;this.camera.position.set(Math.sin(a)*17,8.5,Math.cos(a)*17);this.camera.lookAt(0,2.2,0);}
  },

  runCleanups(){while(this.cleanupFns.length){try{this.cleanupFns.pop()();}catch(e){}}},
  spawnPlayer(x,y,z,setSpawn){
    const P=this.player;P.pos.set(x,y,z);P.vel.set(0,0,0);P.iframe=0;P.onGround=false;P.groundC=null;
  ...
  startGame(meta){
    if(!meta)return;
    this.runId++;this.finished=false;this.gameUpdate=null;this.onFallCb=null;
    this.noMove=!!meta.noMove;this.inputLock=(meta.controls===false);
    this.playerOn=true;this.punch=0;this.player.iframe=0;this.player.speed=7;
    this.camMode=0;this.camDist=9;this.camPitch=0.35;
    W.clear();
    this.mode='game';this.currentMeta=meta;
    HUD.game(meta);HUD.setControls(meta.controls!==false);
...
const Bots={
  all:[],
  names:['xX_Pro_TR_Xx','NoobMaster61','BlokUstası','KralCan_34','Elmas_Efe','RoboAyşe','SpeedyMert','GamerZeynep','DevAdam07','KüçükNinja','TıkCanavarı','YARDIMCI_37'],
  used:[],
  pickName(){const avail=this.names.filter(n=>this.used.indexOf(n)<0);const n=avail.length?U.choice(avail):'Bot'+U.randi(99);this.used.push(n);return n;},
  make(colors,x,z,name){
    const g=new THREE.Group();
    if(!colors){const a=U.choice(CHARS),b=U.choice(CHARS),c=U.choice(CHARS);colors={skin:a.skin,shirt:b.shirt,pants:c.pants};}
    ...
    this.all.push(bot);return bot;
  },
  clear(){this.all.length=0;this.used.length=0;},
  groundAt(x,z,maxFeet){
    let best=null;
    for(let i=0;i<Engine.colliders.length;i++){const c=Engine.colliders[i];
      if(c.disabled||c.trigger)continue;
      const top=c.y+c.hy;if(top>maxFeet+0.6)continue;
      if(Math.abs(x-c.x)<=c.hx+0.35&&Math.abs(z-c.z)<=c.hz+0.35){if(best===null||top>best)best=top;}}
    return best;
  },
  kill(b,delay){if(b.dead)return;b.dead=true;b.respawnT=(delay==null?2.5:delay);b.grp.visible=false;
    FX.burst(b.pos.x,b.feetY+1,b.pos.z,'#ffd32a',12,6,0.7);},
  respawn(b){b.dead=false;b.grp.visible=true;b.pos.copy(b.respawnPos);b.feetY=b.respawnPos.y;b.vy=0;
    if(b.mode==='path'&&b.path)b.idx=b.safeIdx;},
  physics(b,dt){
    b.prevFeet=b.feetY;
    if(b.push.x||b.push.z){b.pos.x+=b.push.x*dt;b.pos.z+=b.push.z*dt;
      b.push.x*=Math.max(0,1-dt*3);b.push.z*=Math.max(0,1-dt*3);
      if(Math.abs(b.push.x)<0.1)b.push.x=0;if(Math.abs(b.push.z)<0.1)b.push.z=0;}
    b.vy=Math.max(-34,b.vy-30*dt);b.feetY+=b.vy*dt;
    const wasG=b.onGround;b.onGround=false;
  ...
  anim(b,dt){
    ...
    b.swing=swing;const L=b.limbs;
    if(b.onGround){L.legL.rotation.x=swing;L.legR.rotation.x=-swing;L.armL.rotation.x=-swing*0.8;
      if(b.punchT<=0)L.armR.rotation.x=swing*0.8;}
    else{L.legL.rotation.x=0.45;L.legR.rotation.x=-0.3;L.armL.rotation.x=-0.5;
      if(b.punchT<=0)L.armR.rotation.x=-0.5;}
    if(b.punchT>0){b.punchT-=dt;L.armR.rotation.x=-2.4*Math.sin(U.clamp(1-b.punchT/0.25,0,1)*Math.PI);}
    if(b.yawT!==null){let d=(b.yawT-b.grp.rotation.y)%(Math.PI*2);
  ...
  walkPath(b,dt){
    ...
    if(b.idx>=b.path.length){
      if(b.loop){b.idx=0;const p0=b.path[0];b.pos.set(p0.x,p0.y,p0.z);b.feetY=p0.y;b.vy=0;b.safePos.copy(b.pos);}
      else{b.hspd=0;return;}}
    const tgt=b.path[b.idx];const dx=tgt.x-b.pos.x,dz=tgt.z-b.pos.z;const d=Math.hypot(dx,dz);
    if(d<0.45){b.safePos.set(tgt.x,tgt.y,tgt.z);b.idx++;b.safeIdx=Math.min(b.idx,b.path.length-1);b.hspd=0;}
    else{const st=Math.min(b.speed*dt,d);b.pos.x+=dx/d*st;b.pos.z+=dz/d*st;
      b.hspd=b.speed;b.yawT=Math.atan2(dx,dz);
      if(b.onGround){const ahead=this.groundAt(b.pos.x+dx/d*1.0,b.pos.z+dz/d*1.0,b.feetY);
        if(tgt.y-b.feetY>0.75||ahead===null)b.vy=12.4;}}
  },
  walkWander(b,dt){
    const w=b.wander;
    if(w.wait>0){w.wait-=dt;b.hspd=0;return;}
    const dx=w.tx-b.pos.x,dz=w.tz-b.pos.z;const d=Math.hypot(dx,dz);
    if(d<0.7){const a=U.rand(0,Math.PI*2),r=U.rand(0.4,1)*w.r;
      w.tx=w.cx+Math.cos(a)*r;w.tz=w.cz+Math.sin(a)*r;w.wait=U.rand(0.6,2.6);b.hspd=0;}
    else{const st=Math.min(b.speed*dt,d);b.pos.x+=dx/d*st;b.pos.z+=dz/d*st;b.hspd=b.speed;b.yawT=Math.atan2(dx,dz);}
    if(b.onGround&&Math.random()<dt*0.12)b.vy=10.5;
  },
  stepTo(b,tx,tz,spd,dt){
    const dx=tx-b.pos.x,dz=tz-b.pos.z;const d=Math.hypot(dx,dz);
    if(d>0.15){const st=Math.min(spd*dt,d);b.pos.x+=dx/d*st;b.pos.z+=dz/d*st;b.hspd=spd;b.yawT=Math.atan2(dx,dz);}
    else b.hspd=0;
    this.physics(b,dt);this.anim(b,dt);return d;
  },
  update(dt){
    for(let i=0;i<this.all.length;i++){const b=this.all[i];
      ...
      this.physics(b,dt);this.anim(b,dt);}
  }
};
