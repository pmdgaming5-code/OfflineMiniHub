/* ============================================================
   games-core.js — Oyun 1-10: Obby, Tower, Lava, Color, Speed,
   Tycoon, Doors, Disaster, Clicker, Boss
   ============================================================ */
'use strict';

/* ============ 1) OBBY ============ */
defGame({id:'obby'}, function(api){
  W.env('#4aa8ff',40,110,'#dff3ff',true);
  W.killY=-8;
  let t=0, done=false, coinN=0, cp={x:0,y:0,z:0};
  ...
  S.forEach((s,i)=>{
    ...
    if(flag===9){
      c.onLand=()=>{
        ...
        api.win(sc, 15+Math.max(0,25-Math.floor(t/4))+coinN*2, 'Süre: '+t.toFixed(1)+' sn');
      };
  ...
  [[2.5,1.4,-10],[5,2.2,-17.5],[-4.5,3.8,-32],[4.5,5.2,-48],[-2,6,-55],[0,7,-64]].forEach(p=>{
    ...
    c.mesh=sp;
    c.cb=()=>{ if(c.disabled)return; coinN++; Sfx.coin(); FX.burst(p[0],p[1],p[2],'#ffd32a',8,4,0.5); W.removeC(c); };
  });
  ...
  api.update(dt=>{ t+=dt; api.stat('⏱ '+t.toFixed(1)+' sn | 🪙 '+coinN); });
});

/* ============ 2) TOWER OF HELL ============ */
defGame({id:'tower'}, function(api){
  W.env('#1b2a55',30,95,'#5a4a7a',false);
  W.killY=-6;
  ...
  api.update(dt=>{
    ...
    api.stat('KAT '+Math.min(N,Math.floor(maxH/1.85))+'/'+N);
  });
});

/* ============ 3) FLOOR IS LAVA ============ */
defGame({id:'lava'}, function(api){
  W.env('#200a0a',18,60,'#7a1f0f',false);
  W.killY=-40;
  ...
  for(let i=1;i<=17;i++){
    ...
    const nx=U.clamp(px+Math.cos(ang)*rr,-9,9);
    const nz=U.clamp(pz+Math.sin(ang)*rr,-9,9);
  ...
  api.update(dt=>{
    ...
    for(let i=0;i<Bots.all.length;i++){
      const b=Bots.all[i];
      if(!b.dead && b.feetY<lavaY+0.15){ b.respawnPos.set(0,0,0); FX.burst(b.pos.x,lavaY+0.4,b.pos.z,'#ff4d00',14,7,0.8); Bots.kill(b,3); }
    }
...
defGame({id:'color'}, function(api){
  ...
  [[-1.2,'MAVICAN_06'],[1.2,'RenkAvı']].forEach(p=>{
    ...
    b.mode='manual'; b.fallBelow=-100; b.speed=6.4; b.goal={x:0,z:0};
    cbots.push(b);
  });
  const pending=[];
  ...
  const die=msg=>{ if(done)return; api.lose(msg+' • '+(round-1)+' tur',(round-1)*100,(round-1)*6); };
  startRound();
  api.spawn(0,0,0);
  ...
  api.update(dt=>{
    ...
    if(phase==='show'&&pt<=0){
      ...
      Sfx.trap();
      cbots.forEach(b=>{
        if(b.dead)return; let safe=false;
...
/* ============ 5) SPEED RUN ============ */
defGame({id:'speed'}, function(api){
  W.env('#12082b',25,80,'#3b1e63',false);
  W.killY=-20;
  ...
  function spawnRow(z){
    ...
    if(type===0){
      ...
      idx.forEach(li=>{
        const c=W.box(lanes[li],1,z,1.9,2,1,'#ef476f',{trigger:true});
  ...
  api.update(dt=>{
    ...
    crashT-=dt;
    if(crashT<=0){ crashT=U.rand(7,12); rivalDist=Math.max(0,rivalDist-7); FX.burst(rivalX,1,-rivalDist,'#ff9f1c',16,7,0.7); Sfx.hit(); }
...
/* ============ 6) MEGA TYCOON ============ */
defGame({id:'tycoon'}, function(api){
  W.env('#8fd3ff',35,110,'#e8f7ff',true);
  W.killY=-30;
  ...
  const cmouth=new THREE.Mesh(Engine.GEO.box,Engine.MATB('#0f1220'));
  ...
  stages.forEach((s,i)=>{
    ...
    const it=api.interact(x,z,2.4,()=>{
      ...
      money-=s.cost; bought++; it.dead=true;
      Engine.scene.remove(pads.pad); Engine.scene.remove(pads.spr);
      Sfx.buy(); api.toast('🏗 '+s.name+' kuruldu!'); FX.burst(x,1,z,'#2ed573',16,6,0.8);
      s.apply();
    });
  });
...
/* ============ 7) DOORS ============ */
defGame({id:'doors'}, function(api){
  W.env('#1a1d26',20,70,'#3a3f4d',false);
  W.killY=-30;
  ...
  function clearRoom(){
    ...
    roomMeshes.forEach(m=>{ if(m&&m.parent)Engine.scene.remove(m); });
...
/* ============ 8) AFET SURVIVAL ============ */
defGame({id:'disaster'}, function(api){
  W.env('#3a2030',25,80,'#8a4a3a',false);
  W.killY=-30;
  let tl=60, hearts=5, inv=0, spawnT=0.8, done=false, boltT=4.5;
  const meteors=[];
  ...
  api.update(dt=>{
    ...
    if(spawnT<=0){ spawnT=iv; spawnMeteor(); if(tl<25&&Math.random()<0.4) spawnMeteor(); }
    if(boltT<=0&&tl<30){ boltT=U.rand(4.5,7); spawnBolt(); }
...
/* ============ 9) CLICKER SIM ============ */
defGame({id:'clicker'}, function(api){
  W.env('#2a1d44',20,60,'#6b4d8a',false);
  W.killY=-30;
  let t=60, power=0, lvl=1, clickPow=1, autoLvl=0, acc=0, up1cost=60, up2cost=150, done=false;
  ...
  api.onExit(()=>{ if(ui)ui.classList.add('hidden'); });
  const myRun=Engine.runId;
  function refreshUI(){
    const pe=$('clk-power'), le=$('clk-lvl'), u1=$('clk-up1c'), u2=$('clk-up2c'), b1=$('clk-up1'), b2=$('clk-up2');
    if(pe)pe.textContent=Math.floor(power);
    if(le)le.textContent=lvl;
    if(u1)u1.textContent='-'+up1cost+' 💪';
    if(u2)u2.textContent=autoLvl>=2?'MAX':'-'+up2cost+' 💪';
    if(b1)b1.classList.toggle('off',power<up1cost);
    if(b2)b2.classList.toggle('off',power<up2cost||autoLvl>=2);
  }
  function checkLevel(){
  ...
  api.update(dt=>{
    if(done)return;
    t-=dt;
    const te=$('clk-time'); if(te)te.textContent=Math.ceil(t);
...
/* ============ 10) BOSS ARENA ============ */
defGame({id:'boss'}, function(api){
  W.env('#1c0b20',25,75,'#5a1f2a',false);
  W.killY=-30;
  Engine.setTool('sword');
  api.onExit(()=>Engine.setTool(null));
  let hearts=5, hp=220, HP0=220, st='chase', stT=0, atkCd=0, done=false, heartT=8;
